<script lang="ts">
    ...
    const yDomainMin = $derived(
        allData.length ? Math.max(0, Math.min(...allData) - 5) : 0
    );
    const yDomainMax = $derived(
        allData.length ? Math.max(...allData) + 5 : 35
    );
    ...
    const yScale = $derived.by(() => {
        ...
        return (value) =>
            range * (1 - (value - yDomainMin) / denom) + PADDING;
    });
    ...
    const points1 = $derived.by(() =>
        data1.map((v, i) => ({ x: xScale(i), y: yScale(v), v }))
    );
    const points2 = $derived.by(() =>
        data2.map((v, i) => ({ x: xScale(i), y: yScale(v), v }))
    );
...
<svg width="100%" height="auto" viewBox={`0 0 ${WIDTH} ${HEIGHT}`}>
    ...
    {#each yLabels as label}
        <line x1={PADDING} y1={label.y} x2={WIDTH - PADDING} y2={label.y}
            stroke="#eee" stroke-dasharray="4 4" />
        <text x={PADDING - 6} y={label.y + 4} text-anchor="end"
            font-size="12" fill="#555">{label.val}</text>
    {/each}
